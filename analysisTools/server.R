...
shinyServer(function(input, output, session) {
    ...
    observe({
        # Update selectInput
        choice_ids <- c("mean_oneFactor_colId", "mean_oneFactor_colValue",
                        "mean_oneFactor_colGroup")
        colx <- rValue_dataSel$colx
        for (ix in choice_ids) {
            updateSelectInput(session, ix, choices = colx)
        }
    ...
    output$mean_oneFactor_normTest <- renderText({
        ...
        if (normality) {
            icon <- icon("check-circle", "fa-2x")
            info <- "Residuals of ANOVA are normally distributed; one-way ANOVA is 
            applicable."
        } else {
            icon <- icon("exclamation-triangle", "fa-2x")
            info <- "Residuals of ANOVA are NOT normally distributed. Kruskal-Wallis 
            rank sum test can be used."
        }
    ...
    output$mean_oneFactor_homoVar <- renderText({
        ...
        line1 <- "<p><b>Levene's Test for Homogeneity of Variance 
            (center = median)</b></p>"
        if (homoVar) {
            icon <- icon("check-circle", "fa-2x")
    ...
    ract_oneFactor_meanTest <- reactive({
        ...
        Aov <- function(df, aov) {
            ...
            # Return
            list(test= test, p_value= p_value, title= title, body= body, 
                 multiComp= multiComp)
        }
        ...
        Fun_test <- switch(method,
                           "auto" = ifelse(normality, ifelse(homoVar, Aov, Oneway),
                                           Kruskal),
                           "one-way ANOVA" = Aov,
    ...
    output$mean_oneFactor_tb_multiComp <- renderDT({
        ...
        if (opt == "p_value") {
            ...
            alpha <-  ract_mean_oneFactor_data()$alphaTest
            tb <- as.data.frame(ract_oneFactor_meanTest()$multiComp$p.value)
            tb <- ifelse(tb >= alpha, "equal", "NOT equal")
        }
