...
shinyServer(function(input, output, session) {
    ...
    observe({
        ...
        choice_ids <- c("mean_oneSample_colSel", 
            "mean_twoSample_colId", "mean_twoSample_colValue", "mean_twoSample_colGroup",
            "mean_pairSample_colId", "mean_pairSample_colValue", "mean_pairSample_colGroup",
            "mean_oneFactor_colId", "mean_oneFactor_colValue", "mean_oneFactor_colGroup")
    ...
    ract_oneSampleMeanTest <- reactive({
        ...
        if (testMethod == "t-test") {
            ...
            df_ci <- data.frame(estimate= mean_test$estimate, 
                               low_ci= mean_test$conf.int[1],
                               up_ci= mean_test$conf.int[2], 
                               fidu= mean_test$null.value)
            li_ci <- list(chart_title= chart_title, legend_name= legend_name, 
                          df_ci= df_ci)
            # power analysis
        ...
        result <- paste("Sample", names(mean_test$null.value), h0, 
                        "Hypothesized", names(mean_test$null.value), sep= " ")
    ...
    output$mean_oneSample_p_ci <- renderPlotly({
        li <- ract_oneSampleMeanTest()$li_ci
        if (is.null(li)) return()
        chart_title = li$chart_title
        legend_name = li$legend_name
        df_ci = li$df_ci
        
        Plotly_ci_one(df_ci, title = chart_title, legend_name = legend_name)
    })
    ...
    ract_twoSampleMeanTest <- reactive({
        ...
        icon <- icon("info-circle", "fa-2x")
        result <- paste0("Sample ", names(mean_test$null.value), " is ", h0, " ",
